...
import in.srini91.learn.pi4j.catalog.components.Buzzer.Note;
...
public class Buzzer extends Component {
	...
	protected static PwmConfig buildPwmConfig(Context pi4j, PIN address) {
		return Pwm.newConfigBuilder(pi4j).id("BCM" + address).name("Buzzer").address(address.getPin())
				.pwmType(PwmType.HARDWARE).initial(0).shutdown(0).build();
	}

	/**
	 * Plays a tone with the given frequency in Hz indefinitely. This method is
	 * non-blocking and returns immediately. A frequency of zero causes the buzzer
	 * to play silence.
	 *
	...
	/**
	 * Plays a tone with the given frequency in Hz for a specific duration. This
	 * method is blocking and will sleep until the specified duration has passed. A
	 * frequency of zero causes the buzzer to play silence. A duration of zero to
	 * play the tone indefinitely and return immediately.
	 *
	 * @param frequency Frequency in Hz
	...
	public void playTone(int frequency, int duration) {
		if (frequency > 0) {
			...
			// This causes the buzzer to be on for half of the time during each cycle,
			// resulting in the desired frequency.
			pwm.on(50, frequency);

			// If the duration is larger than zero, the tone should be automatically stopped
	...
	/**
	 * plays a defined Melody out of Sounds, which are compromised from a frequency
	 * and an amount of beats
	 *
	...
	/**
	 * plays a defined Melody out of Sounds, which are compromised from a frequency
	 * and an amount of beats for several times
	 *
	 * @param tempo       how fast should it be played
	...
	/**
	 * Silences the buzzer and waits for the given duration. This method is blocking
	 * and will sleep until the specified duration has passed.
	 *
	 * @param duration Duration in milliseconds
	 */
	...
	public enum Note {
		...
		AS6(1865), B6(1976), C7(2093), CS7(2217), D7(2349), DS7(2489), E7(2637), F7(2794), FS7(2960), G7(3136),
		GS7(3322), A7(3520), AS7(3729), B7(3951), C8(4186), CS8(4435), D8(4699), DS8(4978), PAUSE(0);
...
/**
 * A Sound is defined through a frequency, that is played for a defined range of
 * beats.
 */
final class Sound {
	final Note note;
	...
	public Sound(Note note, int beats) {
		super();
		this.note = note;
		this.beats = beats;
	}

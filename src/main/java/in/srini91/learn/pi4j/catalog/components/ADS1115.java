...
import in.srini91.learn.pi4j.catalog.components.helpers.ContinuousMeasuringException;
...
public class ADS1115 extends Component {
	 ...
	 * address 0x48 (GND) and max gain 4.096 V (parameters for raspberry pi)
	 *
	 ...
	 *                      lower than 1/2 sampling rate of device
	 */
	...
	/**
	 * Sets or disables the handler for the onValueChange event. This event gets
	 * triggered whenever the analog value from the device changes. Only a single
	 * event handler can be registered at once.
	 *
	 ...
	 * read. This event gets triggered whenever the analog value from the device
	 * changes. Only a single event handler can be registered at once.
	 *
	 * @param method Event handler to call or null to disable
	 */
	 ...
	 * from analog input and triggers valueChange event
	 *
	 * @param config        Configuration for config register
	 * @param threshold     threshold for trigger new value change event
	 * @param readFrequency read frequency to get new value from device, must be
	 *                      lower than the sampling rate of the device
	 */
			...
			new Thread(() -> {
				while (continuousReadingActive) {
					...
					if (oldValue[0] - thresholdDigits > result || oldValue[0] + thresholdDigits < result) {
						logDebug("New event triggered on value change, old value: " + oldValue + " , new value: "
								+ result);
						oldValue[0] = result;
						consumerFastRead.accept(pga.gainPerBit * result);
					}
					// stop measuring time
	 ...
	 * all responses in actualValue array. Waits for the rest of readFrequency time.
	 *
	 * @param threshold     threshold for trigger new value change event
	 * @param readFrequency read frequency to get new value from device, must be
	 *                      lower than 1/2 the sampling rate of the device
	 */
					...
					for (int i = 0; i < numberOfChannels; i++) {
						if (oldValue[i] - thresholdDigits > result[i] || oldValue[i] + thresholdDigits < result[i]) {
							...
							if (consumersSlowRead[i] != null) {
								consumersSlowRead[i].accept(pga.gainPerBit * result[i]);
							}
						}
					}
	...
	/**
	 * I2C Address Selection The ADS1115 has one address pin, ADDR, that configures
	 * the I2C address of the device. This pin can be connected to GND, VDD, SDA, or
	 * SCL, allowing for four different addresses to be selected with one pin. Use
	 * the GND, VDD and SCL addresses first. If SDA is used as the device address,
	 * hold the SDA line low for at least 100 ns after the SCL line goes low to make
	 * sure the device decodes the address correctly during I2C communication.
	 * <p>
	 * Address that can be used {@link #GND} {@link #VDD} {@link #SDA} {@link #SCL}
	 */
	public enum ADDRESS {
		/**
	 ...
	 * ADS1115. The full-scale range is configured by bits PGA[2:0] in the Config
	 * register and can be set to ±6.144 V, ±4.096 V, ±2.048 V, ±1.024 V, ±0.512 V,
	 * ±0.256 V.
	 * <p>
	 * programmable gain amplifier that can be used {@link #GAIN_6_144V}
	 * {@link #GAIN_4_096V} {@link #GAIN_2_048V} {@link #GAIN_1_024V}
	 * {@link #GAIN_0_512V} {@link #GAIN_0_256V}
	 */
	public enum GAIN {
		/**
	 ...
	 * 11, the comparator is disabled and the ALERT/RDY pin is set to a
	 * high-impedance state. When set to any other value, the ALERT/RDY pin and the
	 * comparator function are enabled, and the set value determines the number of
	 * successive conversions exceeding the upper or lower threshold required before
	 * asserting the ALERT/RDY pin.
	 * <p>
	 * teh following comparator queue can be used {@link #ASSERT_ONE}
	 * {@link #ASSERT_TWO} {@link #ASSERT_FOUR} {@link #DISABLE_COMP}
	 */
	public enum COMP_QUE {
		...
		/**
		 * With an AND operation the current parameters will be set to 0 all other
	...
	/**
	 * Latching comparator This bit controls whether the ALERT/RDY pin latches after
	 * being asserted or clears after conversions are within the margin of the upper
	 * and lower threshold values.
	 * <p>
	 * the following mode can be used {@link #NON_LATCH} {@link #DO_LATCH}
	 */
	...
	/**
	 * Comparator polarity This bit controls the polarity of the ALERT/RDY pin.
	 * <p>
	 * The following polarities can be set {@link #ACTIVE_LOW} {@link #ACTIVE_HIGH}
	 */
	...
	/**
	 * Comparator mode This bit configures the comparator operating mode.
	 * <p>
	 * The following modes can be set {@link #TRAD_COMP} {@link #WINDOW_COMP}
	 */
	 ...
	 * <p>
	 * The following modes can be set {@link #SPS_8} {@link #SPS_16} {@link #SPS_32}
	 * {@link #SPS_64} {@link #SPS_128} {@link #SPS_250} {@link #SPS_475}
	 * {@link #SPS_860}
	 */
	public enum DR {
		/**
	 ...
	 * <p>
	 * The following modes can be set {@link #CONTINUOUS} {@link #SINGLE}
	 */
	public enum MODE {
		/**
	...
	/**
	 * Programmable gain amplifier configuration These bits set the FSR of the
	 * programmable gain amplifier.
	 * <p>
	 * The following gain amplifier can be configured {@link #FSR_6_144}
	 * {@link #FSR_4_096} {@link #FSR_2_048} {@link #FSR_1_024} {@link #FSR_0_512}
	 * {@link #FSR_0_256}
	 */
	public enum PGA {
		/**
	 ...
	 * <p>
	 * The following gain amplifier can be configured {@link #AIN0_AIN1}
	 * {@link #AIN0_AIN3} {@link #AIN1_AIN3} {@link #AIN2_AIN3} {@link #AIN0_GND}
	 * {@link #AIN1_GND} {@link #AIN2_GND} {@link #AIN3_GND}
	 */
	public enum MUX {
		/**
	...
	/**
	 * Operational status or single-shot conversion start This bit determines the
	 * operational status of the device. OS can only be written when in power-down
	 * state and has no effect when a conversion is ongoing.
	 * <p>
	 * The following operation can be set {@link #WRITE_START} {@link #READ_CONV}
	 * {@link #READ_NO_CONV}
	 */
	public enum OS {
		/**

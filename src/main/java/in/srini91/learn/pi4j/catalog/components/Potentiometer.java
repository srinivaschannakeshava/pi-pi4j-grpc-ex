...
public class Potentiometer extends Component {
	 ...
	 * triggered whenever the value changes. Only a single event handler can be
	 * registered at once.
	 *
	 ...
	 * to the analog to digital converter. For each device a single read command is
	 * sent to the ad converter and waits for the response. The maximum sampling
	 * frequency of the analog signals depends on how many devices are connected to
	 * the AD converter at the same time. The maximum allowed sampling frequency of
	 * the signal is 1/2 the sampling rate of the ad converter. The reciprocal of
	 * this sampling rate finally results in the minimum response time to a signal
	 * request. (the delay of the bus is not included).
	 * <p>
	 * This leads to the following table for the maximum allowed readFrequency by a
	 * sampling rate of 128 sps: 1 channel in use -> readFrequency max 64Hz (min.
	 * response time = 16ms) 2 channel in use -> readFrequency max 32Hz (min.
	 * response time = 32ms) 3 channel in use -> readFrequency max 21Hz (min.
	 * response time = 48ms) 4 channel in use -> readFrequency max 16Hz (min.
	 * response time = 63ms)
	 *
	 ...
	 * to the ad converter. The maximum allowed readFrequency ist equal to the
	 * sample rate of the ad converter
	 *
	 * @param threshold     threshold for trigger new value change event (+-
	 *                      voltage)
	 * @param readFrequency read frequency to get new value from device, must be

...
public class ServoMotor extends Component {
	...
	/**
	 * Minimum angle of the servo motor used for this instance, should match
	 * previously tested real world values
	 */
	private final float minAngle;
	/**
	 * Maximum angle of the servo motor used for this instance, should match
	 * previously tested real world values
	 */
	private final float maxAngle;
	/**
	 * Minimum duty cycle of the servo motor for this instance, should match
	 * previously tested real world values
	 */
	private final float minDutyCycle;
	/**
	 * Maximum duty cycle of the servo motor for this instance, should match
	 * previously tested real world values
	 */
	...
	/**
	 * Creates a new step motor component with the default pin, angle range as well
	 * as duty cycle range.
	 *
	...
	/**
	 * Creates a new step motor component with the default pin and frequency but
	 * customized angle and duty cycle values.
	 *
	 * @param pi4j         Pi4J context
	 * @param address      Custom BCM pin address
	 ...
	 */
	public ServoMotor(Context pi4j, PIN address, float minAngle, float maxAngle, float minDutyCycle,
			float maxDutyCycle) {
		this(pi4j, address, DEFAULT_FREQUENCY, minAngle, maxAngle, minDutyCycle, maxDutyCycle);
	}

	/**
	 * Creates a new step motor component with custom pin, frequency, angle range
	 * and duty cycle range values.
	 *
	 * @param pi4j         Pi4J context
	 * @param address      Custom BCM pin address
	 ...
	 * @param maxDutyCycle Maximum duty cycle as float, between 0 and 100
	 */
	public ServoMotor(Context pi4j, PIN address, int frequency, float minAngle, float maxAngle, float minDutyCycle,
			float maxDutyCycle) {
	...
	/**
	 * Rotates the servo motor to the specified angle in degrees. The angle should
	 * be between {@link #getMinAngle()} and {@link #getMaxAngle()} which was
	 * specified during initialization. Values outside this inclusive range are
	 * automatically being clamped to their respective minimum / maximum.
	 *
	 ...
	 * {@link #getMinAngle()} and {@link #getMaxAngle()}. As an example, a value of
	 * 0% will equal to the minimum angle, 50% to the center and 100% to the maximum
	 * angle.
	 *
	 ...
	 * {@link #setRange(float, float)} to the full range of the servo. If
	 * {@link #setRange(float, float)} was not called before, the default range of
	 * 0-1 (as float) is being used.
	 *
	...
	/**
	 * Maps the given value based on the given input range to the full range of the
	 * servo. Unlike {@link #moveOnRange(float)}, this method will NOT use or adjust
	 * the values set by {@link #setRange(float, float)}.
	 *
	 * @param value    Value to map
	...
	/**
	 * Adjusts the minimum and maximum for the user-defined range which can be used
	 * in combination with {@link #moveOnRange(float)}. This method will only affect
	 * future calls to {@link #moveOnRange(float)} and does not change the current
	 * position.
	 *
	 * @param minValue Minimum range value
	 * @param maxValue Maximum range value
	...
	/**
	 * Helper function to map an angle between {@link #minAngle} and
	 * {@link #maxAngle} to the configured duty cycle range.
	 *
	...
	/**
	 * Helper function to map an input value between a specified range to the
	 * configured duty cycle range.
	 *
	...
	/**
	 * Helper function to map an input value from its input range to a possibly
	...
	protected PwmConfig buildPwmConfig(Context pi4j, PIN address, int frequency) {
		return Pwm.newConfigBuilder(pi4j).id("BCM" + address).name("Servo Motor").address(address.getPin())
				.frequency(frequency).initial(0).shutdown(0).build();
	}
}

...
/**
 * Implementation of a LCDDisplay using GPIO with Pi4J For now, only works with
 * the PCF8574T Backpack
 */
public class LCDDisplay extends Component {
	...
	private I2CConfig buildI2CConfig(Context pi4j, int bus, int device) {
		return I2C.newConfigBuilder(pi4j).id("I2C-" + device + "@" + bus).name("PCF8574AT").bus(bus).device(device)
				.build();
	}
	 ...
	 *
	 * @param blink Blink = true means the cursor will change to blinking mode.
	 *              False lets the cursor stay static
	 */
	 ...
	 * {@link InterruptedException} will be catched and ignored while setting the

...
public class SimpleButton extends Component {
	...
	public SimpleButton(Context pi4j, PIN address, boolean inverted, long debounce) {
		...
		/*
		 * Gets a DigitalStateChangeEvent directly from the Provider, as this Class is a
		 * listener. This runs in a different Thread than main. Calls the mehtods onUp,
		 * onDown and whilePressed. WhilePressed gets executed in an own Thread, as to
		...
		this.digitalInput.addListener(digitalStateChangeEvent -> {
			...
			case UNKNOWN:
				logError("Button is in State UNKNOWN");
			}
		});
	}
	...
	private DigitalInputConfig buildDigitalInputConfig(Context pi4j, PIN address, boolean inverted, long debounce) {
		return DigitalInput.newConfigBuilder(pi4j).id("BCM" + address).name("Button #" + address)
